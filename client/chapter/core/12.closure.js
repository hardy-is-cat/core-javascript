...
first();
// 자스는 함수를 값으로 인식하기 때문에 second 함수가 나온다 second를 실행하고 싶으면 first()()
// 하지만 보통 ()() 하진 않고 first()를 변수에 할당해서 사용한다
const value = first();
value();
...
//   return ++count;
// }
// count가 전역 변수이기 때문에 다른 곳에서 count를 재정의 해버리면 함수가 소용 없어진다
...
const counter = count();
// 따라서 위의 예제와 같이 A함수 안에 A변수를 넣고 B함수를 넣은 다음에 B함수를 반환하고
// A함수의 실행 값을 B변수에 할당하면 A변수가 전역을 오염시키지 않게 된다
// 그런데 B함수에 이름을 줄 필요는 없다 어차피 값만 나오면 되니까 게다가 화살표 함수로 써도 됨
// 대신 this를 쓰려면 화살표함수 말고 일반 함수로 사용 해야 됨
